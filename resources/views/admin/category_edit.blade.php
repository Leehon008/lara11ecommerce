...
@section('content')
    <div class="main-content-inner">
        <div class="main-content-wrap">
            ...
            <!-- new-category -->
            <div class="wg-box">
                <form class="form-new-product form-style-1" action="{{ route('admin.category_update') }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <input type="text" name="id" id="id" hidden value="{{ $category->id }}" />
                    <fieldset class="name">
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow" type="text" placeholder="Category Description" name="description"
                            tabindex="0" value="{{ $category->description }}" aria-required="true" required="">
                        @error('description')
                            <span class="alert alert-danger text-center">{{ $message }} </span>
                        @enderror
                    </fieldset>
